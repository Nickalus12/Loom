...
Loom maintains session state under `docs/loom` (resolved from `LOOM_STATE_DIR`):
...
- `node ${CLAUDE_PLUGIN_ROOT}/scripts/ensure-workspace.js docs/loom` â€” initialize workspace directories
- `node ${CLAUDE_PLUGIN_ROOT}/scripts/read-active-session.js` â€” read current session state
- `node ${CLAUDE_PLUGIN_ROOT}/scripts/read-state.js <relative-path>` â€” read arbitrary state file
- `node ${CLAUDE_PLUGIN_ROOT}/scripts/write-state.js <relative-path>` â€” write state from stdin
- `node ${CLAUDE_PLUGIN_ROOT}/scripts/read-setting.js <SETTING_NAME>` â€” resolve a Loom setting
