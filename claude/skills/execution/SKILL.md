...
- `orchestrator inline validation (no hook â€” see SKIP_EVENTS_CLAUDE)`: validates that the response contains both `Task Report` and `Downstream Context`; requests one retry on the first malformed response
